# === 完整的优化版代码（实现动态对冲比率 + 动态协整监控 + 自适应交易带 + 无交易区间 + VaR头寸管理 + 因子中性化）===
# === 第一部分：导入必要的库 ===
import struct
import pandas as pd
import numpy as np
...
# === 第三部分：新增因子数据获取函数 ===
def get_stock_factor_data(stock_code_list, factor_type='daily'):
    """
    获取股票的风格因子数据（简化版，实际应用中可能需要从Wind、JoinQuant等专业数据源获取）
    这里使用Tushare获取部分因子数据
    """
    factor_data = {}
    ...
    for stock_code in stock_code_list:
        try:
            # 获取股票基本信息（简化处理，实际应用中应获取市值、估值等因子）
            stock_info = ak.stock_individual_info_em(symbol=f"{stock_code}")
            ...
            if stock_info is not None and not stock_info.empty:
                ...
                for _, row in stock_info.iterrows():
                    if row['item'] == '总市值':
                        try:
                            market_cap = float(row['value'].replace('亿', '')) * 1e8
                        except:
                            pass
                    elif row['item'] == '市盈率(动态)':
                        try:
            ...
            print(f"获取股票{stock_code}因子数据失败: {e}")
            factor_data[stock_code] = {
                'market_cap': None,
                'pe_ratio': None,
                'pb_ratio': None
...
def calculate_factor_distance(stock1_factors, stock2_factors):
    """
    计算两只股票的因子距离
    参数:
        stock1_factors: 股票1的因子数据字典
        stock2_factors: 股票2的因子数据字典
    返回:
        因子距离（0-1之间，越小表示因子越相似）
    """
    if not stock1_factors or not stock2_factors:
        return 1.0  # 无法计算，返回最大距离
    
    # 检查所有因子是否存在
    factors_to_check = ['market_cap', 'pe_ratio', 'pb_ratio']
    valid_factors = []
    factor_diffs = []
    
    for factor in factors_to_check:
        val1 = stock1_factors.get(factor)
        val2 = stock2_factors.get(factor)
        
        if val1 is not None and val2 is not None and not np.isnan(val1) and not np.isnan(val2):
            valid_factors.append(factor)
            # 对数值取对数处理，使其更稳定
            if factor == 'market_cap':
                # 市值取对数差
                diff = abs(np.log(val1 + 1) - np.log(val2 + 1))
            else:
                # 其他因子直接计算相对差异
                diff = abs(val1 - val2) / (abs(val1) + abs(val2) + 1e-6)
            factor_diffs.append(diff)
    
    if not valid_factors:
        return 1.0
    
    # 计算加权平均距离
    weights = {'market_cap': 0.5, 'pe_ratio': 0.3, 'pb_ratio': 0.2}
    weighted_diffs = []
    
    for factor, diff in zip(valid_factors, factor_diffs):
        if factor in weights:
            weighted_diffs.append(diff * weights[factor])
    
    if not weighted_diffs:
        return 1.0
    
    factor_distance = np.mean(weighted_diffs)
    
    # 归一化到0-1之间
    factor_distance = min(1.0, factor_distance)
    
    return factor_distance

# === 第四部分：A股适配的强弱轮动策略（优化版v3）===
class ASharePairsTradingStrategyOptimizedV3:
    """A股配对交易策略（强弱轮动，不允许融券）- 优化版v3
    新增功能：
    1. 自适应交易带：入场/出场阈值与价差滚动标准差挂钩
    2. 无交易区间：平仓后设置冷却期，防止频繁交易
    3. 基于VaR的头寸管理：根据价差风险动态调整仓位
    4. 因子中性化检查：在配对筛选中考虑风格因子暴露
    """
    def __init__(self, initial_capital=1000000, commission_rate=0.0003, 
                 ...
                 entry_threshold_multiplier=2.0,  # 优化点3：入场阈值乘数
                 exit_threshold_multiplier=0.5,   # 优化点3：出场阈值乘数
                 stop_loss_multiplier=2.5,        # 优化点3：止损阈值乘数
                 cooling_period=5,                # 优化点4：冷却期（交易日）
                 rebalance_threshold_multiplier=0.8,  # 轮动阈值乘数
                 use_adaptive_thresholds=True,    # 是否使用自适应阈值
                 use_var_position_sizing=True,    # 优化点5：是否使用VaR头寸管理
                 var_confidence_level=0.95,       # 优化点5：VaR置信水平
                 var_window=60,                   # 优化点5：VaR计算窗口
                 max_risk_budget=0.1,             # 优化点5：最大风险预算
                 factor_distance_threshold=0.3,   # 优化点6：因子距离阈值
                 factor_neutral_check=True        # 优化点6：是否进行因子中性化检查
                 ):
        ...
        # 优化点5：VaR头寸管理参数
        self.use_var_position_sizing = use_var_position_sizing
        ...
        # 优化点6：因子中性化参数
        self.factor_distance_threshold = factor_distance_threshold
        self.factor_neutral_check = factor_neutral_check
        self.factor_distance = 0  # 记录因子距离
    ...
    def calculate_var_and_cvar(self, spread_returns, confidence_level=0.95):
        """
        优化点5：计算VaR和CVaR
        参数:
            spread_returns: 价差收益率序列
        ...
        # 方法1：历史模拟法
        var_hist = -np.percentile(spread_returns, (1 - confidence_level) * 100)
        
        # 方法2：参数法（假设正态分布）
        mean_ret = np.mean(spread_returns)
        std_ret = np.std(spread_returns)
        var_param = -(mean_ret + std_ret * norm.ppf(confidence_level))
        ...
        # 计算CVaR（条件VaR）
        threshold = -var_value
        exceed_losses = spread_returns[spread_returns <= -var_value]
        if len(exceed_losses) > 0:
            cvar_value = -np.mean(exceed_losses)
        else:
            cvar_value = var_value * 1.3  # 保守估计
        
        return var_value, cvar_value

    def calculate_var_based_position(self, total_value, var_value, cvar_value):
        """
        优化点5：基于VaR计算头寸规模
        参数:
            total_value: 投资组合总价值
        ...
        返回:
            基于风险调整的头寸规模比例
        """
        if not self.use_var_position_sizing or var_value <= 0:
            return self.max_position_ratio
        
        # 计算单位风险暴露
        # 使用CVaR作为风险度量（比VaR更保守）
        risk_measure = cvar_value
        
        # 计算风险调整后的头寸比例
        # 风险越高，头寸比例越低
        risk_adjusted_ratio = min(self.max_risk_budget / (risk_measure + 1e-6), self.max_position_ratio)
        
        # 确保不低于最小头寸比例
        risk_adjusted_ratio = max(risk_adjusted_ratio, self.min_position_ratio)
        
        return risk_adjusted_ratio
    ...
    def calculate_adaptive_thresholds(self, spread_std, entry_threshold=1.5, exit_threshold=0.3, 
        ...
        # 使用固定阈值作为基础，乘以价差标准差的调整系数
        adaptive_entry = self.entry_threshold_multiplier
        adaptive_exit = self.exit_threshold_multiplier
        adaptive_stop_loss = self.stop_loss_multiplier
        adaptive_rebalance = self.rebalance_threshold_multiplier
        
        return adaptive_entry, adaptive_exit, adaptive_stop_loss, adaptive_rebalance

    def update_cooling_period(self, date, action=None):
        """
        优化点4：更新无交易区间状态
        参数:
            date: 当前日期
    ...
    def execute_strategy(self, stock1_code, stock2_code, stock1_data, stock2_data, 
                        ...
                        factor_distance=0.0):  # 新增：因子距离参数
        """
        执行强弱轮动策略 (优化版v3，增加VaR头寸管理和因子中性化)
        ...
        self.last_exit_date = None
        self.in_cooling_period = False
        self.factor_distance = factor_distance  # 记录因子距离
        
        # 优化点6：检查因子距离是否超过阈值
        if self.factor_neutral_check and factor_distance > self.factor_distance_threshold:
            print(f"  ⚠️ 因子距离{self.factor_distance:.3f}超过阈值{self.factor_distance_threshold:.3f}，跳过该配对")
            return None
        ...
        # 优化点5：计算价差收益率用于VaR计算
        spread_pct = spread.pct_change().dropna()
        ...
        for i, date in enumerate(trading_dates):
            ...
            var_value = 0
            cvar_value = 0
            if self.use_var_position_sizing and len(spread_pct) > 0:
                # 使用最近的价差收益率计算VaR
                if i >= self.var_window:
                    recent_returns = spread_pct.iloc[i-self.var_window:i]
                else:
                    recent_returns = spread_pct.iloc[:i] if i > 0 else pd.Series()
                
                if len(recent_returns) > 20:
                    var_value, cvar_value = self.calculate_var_and_cvar(recent_returns.values, self.var_confidence_level)
                    self.current_var = var_value
                    self.current_cvar = cvar_value
            ...
            if not self.pair_valid:
                ...
                daily_record = {
                    ...
                    'factor_distance': self.factor_distance
                }
            ...
            if holding_stock is None:
                ...
                # 优化点5：计算基于VaR的头寸比例
                if self.use_var_position_sizing and var_value > 0:
                    position_ratio = self.calculate_var_based_position(total_value, var_value, cvar_value)
                else:
                    position_ratio = self.max_position_ratio
                
                if z > adaptive_entry:  # stock1相对高估，stock2低估
                    position_value = min(total_value * position_ratio, self.capital)
                    volume = int(position_value / price2 / 100) * 100
                    
                    if volume > 0:
                        success, _, _ = self.buy_stock(date, stock2_code, price2, volume, 
                                                      reason=f"开仓-买入低估股({stock2_code}), z={z:.2f}, 入场阈值={adaptive_entry:.2f}, VaR头寸比例={position_ratio:.2%}")
                        if success:
                    ...
                    if volume > 0:
                        success, _, _ = self.buy_stock(date, stock1_code, price1, volume, 
                                                      reason=f"开仓-买入低估股({stock1_code}), z={z:.2f}, 入场阈值={adaptive_entry:.2f}, VaR头寸比例={position_ratio:.2%}")
                        if success:
            ...
            # 情况2：持有股票2
                ...
                if z < -adaptive_rebalance:  # stock1变得低估
                    if has_stock2:
                        pos = self.positions[stock2_code]
                        sell_volume = pos['qty']
                        if sell_volume > 0:
                            ...
                            # 优化点4：记录平仓，开始冷却期
                            self.update_cooling_period(date, action="exit")
                    ...
                    if not self.in_cooling_period:
                        ...
                        if volume > 0:
                            success, _, _ = self.buy_stock(date, stock1_code, price1, volume,
                                                          reason=f"轮动-买入低估股, z={z:.2f}, 轮动阈值={adaptive_rebalance:.2f}, VaR头寸比例={position_ratio:.2%}")
                            if success:
                                holding_stock = 'stock1'
                                entry_date = date
                                entry_zscore = z
                ...
                elif abs(z) > adaptive_stop_loss:  # 止损
                    if has_stock2:
                        pos = self.positions[stock2_code]
                        sell_volume = pos['qty']
                        if sell_volume > 0:
                            self.sell_stock(date, stock2_code, price2, sell_volume,
                                          reason=f"平仓-止损, z={z:.2f}, 止损阈值={adaptive_stop_loss:.2f}")
                            # 优化点4：记录平仓，开始冷却期
                            self.update_cooling_period(date, action="exit")
                    holding_stock = None
                    entry_date = None
            ...
            # 情况3：持有股票1
                ...
                if z > adaptive_rebalance:  # stock2变得低估
                    if has_stock1:
                        pos = self.positions[stock1_code]
                        sell_volume = pos['qty']
                        if sell_volume > 0:
...
def calculate_pair_parameters_in_sample(stock1_data, stock2_data, window_start, window_end, 
    ...
    # 计算因子距离
    factor_distance = 0
    if stock1_factors is not None and stock2_factors is not None:
        factor_distance = calculate_factor_distance(stock1_factors, stock2_factors)
        print(f"  因子距离: {factor_distance:.4f}")
    
    return hedge_ratio, intercept, coint_pvalue, adf_pvalue, spread_mean, spread_std, factor_distance

# === 第五步：主程序（优化版v3，包含VaR头寸管理和因子中性化）===
if __name__ == "__main__":
    ...
    OUT_SAMPLE_END = "2026-03-30"    # 样本外结束
    ...
    # 自适应交易带参数
    ENTRY_MULTIPLIER = 1.5    # 入场阈值乘数
    EXIT_MULTIPLIER = 0.3     # 出场阈值乘数
    STOP_LOSS_MULTIPLIER = 2.0  # 止损阈值乘数
    REBALANCE_MULTIPLIER = 0.8  # 轮动阈值乘数
    ...
    # VaR头寸管理参数
    USE_VAR_SIZING = True     # 是否使用VaR头寸管理
    VAR_CONFIDENCE = 0.95     # VaR置信水平
    VAR_WINDOW = 60           # VaR计算窗口
    MAX_RISK_BUDGET = 0.1     # 最大风险预算
    
    # 因子中性化参数
    FACTOR_DISTANCE_THRESHOLD = 0.3  # 因子距离阈值
    FACTOR_NEUTRAL_CHECK = True      # 是否进行因子中性化检查
    
    print("="*60)
    print("A股配对交易策略（优化版v3：动态对冲比率 + 动态协整监控 + 自适应交易带 + 无交易区间 + VaR头寸管理 + 因子中性化）")
    print(f"目标行业: {TARGET_INDUSTRY}")
    print(f"只回测排名前{TOP_N_PAIRS}的配对")
    print(f"自适应阈值: {'启用' if USE_ADAPTIVE else '禁用'}")
    print(f"VaR头寸管理: {'启用' if USE_VAR_SIZING else '禁用'}")
    print(f"因子中性化检查: {'启用' if FACTOR_NEUTRAL_CHECK else '禁用'}")
    ...
    print(f"VaR置信水平: {VAR_CONFIDENCE}, VaR窗口: {VAR_WINDOW}")
    print(f"因子距离阈值: {FACTOR_DISTANCE_THRESHOLD}")
    print("="*60)
    ...
    # 优化点6：获取股票因子数据
    print(f"\n获取股票因子数据（市值、估值等）...")
    factor_data = get_stock_factor_data(industry_stocks)
    factor_stocks = [code for code in industry_stocks if code in factor_data]
    print(f"成功获取 {len(factor_stocks)} 只股票的因子数据")
    ...
    for stock_code in factor_stocks:  # 只使用有因子数据的股票
    ...
    for stock1, stock2, ssd_dist, correlation in candidate_pairs[:20]:
        ...
        # 获取因子数据
        stock1_factors = factor_data.get(stock1)
        stock2_factors = factor_data.get(stock2)
        
        # 在样本内计算配对参数（包含因子距离）
        result = calculate_pair_parameters_in_sample(
            data1_full, data2_full, IN_SAMPLE_START, IN_SAMPLE_END,
            stock1_factors, stock2_factors
        )
        
        if result[0] is None:
            continue
        
        hedge_ratio, intercept, coint_p, adf_p, spread_mean, spread_std, factor_distance = result
        
        # 筛选条件
        if correlation > 0.90 and coint_p < 0.05 and adf_p < 0.05:
            # 优化点6：因子距离检查
            if FACTOR_NEUTRAL_CHECK and factor_distance > FACTOR_DISTANCE_THRESHOLD:
                print(f"  ✗ 因子距离过大: {stock1} - {stock2}, 因子距离={factor_distance:.4f} > 阈值={FACTOR_DISTANCE_THRESHOLD}")
                continue
            ...
            print(f"    SSD距离: {ssd_dist:.1f}, 相关性: {correlation:.3f}, 因子距离: {factor_distance:.4f}")
    ...
    for i, pair in enumerate(top_pairs):
        print(f"{i+1}. {pair['stock1']} - {pair['stock2']}: SSD={pair['ssd']:.1f}, "
              f"相关性={pair['corr']:.3f}, 对冲比率={pair['hedge_ratio']:.4f}, 因子距离={pair['factor_distance']:.4f}")
    ...
    sample_in_df = pd.DataFrame([{
        ...
        '价差均值': p['spread_mean'],
        '价差标准差': p['spread_std'],
        '因子距离': p['factor_distance']
    } for p in selected_pairs_with_params])
    sample_in_df.to_csv(f'A股配对_样本内筛选结果_优化版v3_前{TOP_N_PAIRS}对.csv', index=False, encoding='utf-8-sig')
    print(f"样本内筛选结果已保存到: A股配对_样本内筛选结果_优化版v3_前{TOP_N_PAIRS}对.csv")
    ...
    for idx, pair in enumerate(top_pairs):
        ...
        factor_distance = pair['factor_distance']
        
        print(f"\n回测第 {idx+1}/{len(top_pairs)} 对: {stock1} - {stock2}")
        print(f"对冲比率: {hedge_ratio:.4f}, 样本内协整p值: {coint_p:.4f}, 因子距离: {factor_distance:.4f}")
        
        # 创建策略实例，传入训练期计算出的参数和新优化参数
        strategy = ASharePairsTradingStrategyOptimizedV3(
            ...
            use_var_position_sizing=USE_VAR_SIZING,
            var_confidence_level=VAR_CONFIDENCE,
            var_window=VAR_WINDOW,
            max_risk_budget=MAX_RISK_BUDGET,
            factor_distance_threshold=FACTOR_DISTANCE_THRESHOLD,
            factor_neutral_check=FACTOR_NEUTRAL_CHECK
        )
        ...
        if result is not None:
            ...
            trades_df.to_csv(f'A股配对_交易记录_{stock1}_{stock2}_优化版v3_前{TOP_N_PAIRS}对.csv', index=False, encoding='utf-8-sig')
            
            # 保存每日净值
            if strategy.equity_curve:
                equity_df = pd.DataFrame(strategy.equity_curve, columns=['date', 'value'])
                equity_df.set_index('date', inplace=True)
                equity_df.to_csv(f'A股配对_每日净值_{stock1}_{stock2}_优化版v3_前{TOP_N_PAIRS}对.csv', encoding='utf-8-sig')
    
    # 输出汇总结果
    if all_results:
        print(f"\n" + "="*60)
        print(f"样本外回测汇总结果（优化版v3，前{TOP_N_PAIRS}对）")
        print("="*60)
        ...
        for i, r in enumerate(sorted_results):
            ...
            print(f"{i+1}. {r['pair']}: 收益率={r['return']:.2%}, 对冲比率={r['hedge_ratio']:.4f}, "
                  f"因子距离={r['factor_distance']:.4f}, 交易次数={r['trades']}, 胜率={r['win_rate']:.1f}%{cooling_str}, 状态={status}")
        ...
        plt.title(f'{TARGET_INDUSTRY}行业配对交易策略样本外回测（优化版v3，前{TOP_N_PAIRS}对）\n'
                  f'动态对冲比率 + 动态协整监控 + 自适应交易带 + 无交易区间 + VaR头寸管理 + 因子中性化 | 总收益率: {overall_return:.2%}', 
        ...
        plt.tight_layout()
        plt.savefig(f'{TARGET_INDUSTRY}配对_样本外回测净值曲线_优化版v3_前{TOP_N_PAIRS}对.png', dpi=300, bbox_inches='tight')
        plt.show()
        ...
        summary_df.to_csv(f'{TARGET_INDUSTRY}配对_样本外回测汇总_优化版v3_前{TOP_N_PAIRS}对.csv', index=False, encoding='utf-8-sig')
        print(f"\n详细结果已保存到: {TARGET_INDUSTRY}配对_样本外回测汇总_优化版v3_前{TOP_N_PAIRS}对.csv")
        ...
        if len(all_results) > 1:
            ...
            # 计算因子距离与收益的相关系数
            factor_distances = [r['factor_distance'] for r in all_results]
            returns = [r['return'] for r in all_results]
            
            if len(factor_distances) > 1 and len(returns) > 1:
                corr_factor_return = np.corrcoef(factor_distances, returns)[0, 1]
                print(f"因子距离与收益的相关系数: {corr_factor_return:.4f}")
                
                if corr_factor_return < -0.3:
                    print(f"  ✓ 因子距离与收益呈较强负相关，说明因子中性化有效")
                elif corr_factor_return > 0.3:
                    print(f"  ⚠️ 因子距离与收益呈较强正相关，需重新评估因子中性化标准")
                else:
                    print(f"  → 因子距离与收益相关性较弱")
            
            # 按因子距离分组
            low_factor_pairs = [r for r in all_results if r['factor_distance'] < 0.1]
            medium_factor_pairs = [r for r in all_results if 0.1 <= r['factor_distance'] < 0.2]
            high_factor_pairs = [r for r in all_results if r['factor_distance'] >= 0.2]
            
            print(f"\n按因子距离分组统计:")
            print(f"  低因子距离组(<0.1): {len(low_factor_pairs)}对, 平均收益: {np.mean([r['return'] for r in low_factor_pairs]):.2%}" if low_factor_pairs else "  低因子距离组: 无配对")
            print(f"  中因子距离组(0.1-0.2): {len(medium_factor_pairs)}对, 平均收益: {np.mean([r['return'] for r in medium_factor_pairs]):.2%}" if medium_factor_pairs else "  中因子距离组: 无配对")
            print(f"  高因子距离组(≥0.2): {len(high_factor_pairs)}对, 平均收益: {np.mean([r['return'] for r in high_factor_pairs]):.2%}" if high_factor_pairs else "  高因子距离组: 无配对")
    else:
        print("\n没有成功的回测结果")