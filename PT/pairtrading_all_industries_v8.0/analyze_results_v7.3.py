...
def load_latest_results():
    """加载最新的v7.3回测结果文件"""
    # 优先v7.3格式
    files = glob.glob("v7.3_组合回测结果_*.csv")
    if not files:
        # 回退v7.1格式
...
def plot_portfolio_nav_curve(df):
    ...
    plt.tight_layout()
    plt.savefig('v7.3_组合净值走势分析.png', dpi=300, bbox_inches='tight')
    print("✓ 图表已保存: v7.3_组合净值走势分析.png")
    plt.show()
...
def plot_portfolio_nav_curve_simplified(df):
    ...
    plt.tight_layout()
    plt.savefig('v7.3_组合净值走势分析_估算.png', dpi=300, bbox_inches='tight')
    print("✓ 图表已保存: v7.3_组合净值走势分析_估算.png（注意：基于估算数据）")
    plt.show()
...
def plot_adaptive_params_analysis_v72(df):
    ...
    if 'halflife' not in df.columns:
        print("⚠️ 非v7.2/v7.3格式，跳过自适应参数分析")
        return
    ...
    plt.tight_layout()
    plt.savefig('v7.3_自适应参数分析.png', dpi=300, bbox_inches='tight')
    print("✓ 图表已保存: v7.3_自适应参数分析.png")
    plt.show()

def plot_industry_weights_v72(df):
    """v7.3专属：申万行业权重分配可视化（支持单行业情况）"""
    plt.rcParams['font.sans-serif'] = ['Microsoft YaHei', 'SimHei', 'SimSun', 'FangSong', 'KaiTi']
    plt.rcParams['axes.unicode_minus'] = False
    ...
    if n_industries == 1:
        ...
        plt.tight_layout()
        plt.savefig('v7.3_申万行业权重分配.png', dpi=300, bbox_inches='tight')
        print("✓ 图表已保存: v7.3_申万行业权重分配.png（单行业模式）")
        plt.show()
        return
...
def plot_slippage_analysis_v72(df):
    ...
    ax3.set_title('各申万二级行业平均滑点成本（v7.3目标<0.8%）', fontsize=14, fontweight='bold')
    ax3.axvline(x=0.8, color='green', linestyle='--', alpha=0.5, label='v7.3目标线')
    ...
    plt.tight_layout()
    plt.savefig('v7.3_滑点影响分析.png', dpi=300, bbox_inches='tight')
    print("✓ 图表已保存: v7.3_滑点影响分析.png")
    plt.show()
...
def plot_return_distribution_v72(df):
    ...
    ax2.set_xlabel('夏普比率')
    ax2.set_ylabel('频数')
    ax2.set_title('夏普比率分布（v7.3目标>0.85）')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig('v7.3_收益风险分布分析.png', dpi=300, bbox_inches='tight')
    print("✓ 图表已保存: v7.3_收益风险分布分析.png")
    plt.show()
...
def plot_top_pairs_table_v72(df, top_n=20):
    ...
    plt.title(f'Top {top_n} 配对排名（v7.3申万行业自适应策略）', 
             fontsize=16, fontweight='bold', pad=20)
    plt.savefig(f'v7.3_Top{top_n}_配对排名表.png', dpi=300, bbox_inches='tight')
    print(f"✓ 图表已保存: v7.3_Top{top_n}_配对排名表.png")
    plt.show()
...
if __name__ == "__main__":
    ...
    # 【v7.2修复】组合净值走势 - 优先使用实际数据
    print("\n[v7.3核心] 组合净值走势分析...")
    plot_portfolio_nav_curve(df)

    if version in ["v7.3", "v7.2"]:
        print("\n[v7.3专属] 自适应参数分析...")
    ...
    print("\n✓ 全部分析完成！")
    print(f"\n生成图表清单（{version} - 申万行业）：")
    print("  - v7.3_组合净值走势分析.png 【修复：基于实际数据】")
    if version in ["v7.3", "v7.2"]:
        print("  - v7.3_自适应参数分析.png")
        print("  - v7.3_申万行业权重分配.png")
        print("  - v7.3_滑点影响分析.png")
    print("  - v7.3_收益风险分布分析.png")
    print(f"  - v7.3_Top20_配对排名表.png")