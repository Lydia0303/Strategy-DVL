...
CONFIG = {
    ...
    # 【v7.4新增】行业动量轮动参数
    'MOMENTUM_ROTATION_PARAMS': {
        'momentum_lookback': 60,       # 动量计算回看窗口
        'rebalance_freq': 20,          # 再平衡频率（交易日）
        'momentum_threshold': 0.0,     # 动量入选门槛
        'max_rotation_ratio': 0.3,     # 单次最大轮动比例
        'volatility_penalty': 0.2,     # 波动率惩罚系数
        'min_holding_days': 5,         # 最小持有天数
        'max_industries': 10           # 最大行业数量
    },
...
class IndustryMomentumRotator:
    ...
    def __init__(self, 
        ...
        self.max_rotation_ratio = max_rotation_ratio
        self.volatility_penalty = volatility_penalty
        self.min_holding_days = min_holding_days
        self.max_industries = max_industries
        
        # 轮动跟踪状态
        self.current_holdings = {}  # {行业名: {'entry_date': date, 'momentum_score': score, ...}}
        self.rotation_log = []
        self.last_rebalance_date = None
    
    def calculate_momentum_scores(self, industry_returns_dict, current_date):
        """
        计算各行业动量分数
        动量 = 近期收益 - 波动率惩罚
        """
        ...
        for industry_name, returns in industry_returns_dict.items():
            if len(returns) < self.momentum_lookback:
                continue
                
            # 取最近 momentum_lookback 个交易日的收益
            recent_returns = returns[-self.momentum_lookback:]
            
            # 计算动量指标
            recent_return = np.mean(recent_returns[-20:])  # 最近20天收益
            volatility = np.std(recent_returns)  # 波动率
            
            # 动量分数 = 近期收益 - 波动率惩罚
            momentum_score = recent_return - self.volatility_penalty * volatility
            
            # 计算额外指标用于决策
            sharpe_ratio = recent_return / volatility if volatility > 0 else 0
            win_rate = np.sum(recent_returns > 0) / len(recent_returns)
            
            momentum_scores[industry_name] = {
    ...
    def should_rebalance(self, current_date, industry_results):
        """
        判断是否应该执行再平衡
        """
        ...
        # 动量衰减检查：检查当前持仓行业的动量是否显著下降
        momentum_decay_count = 0
        for industry_name, holding_info in self.current_holdings.items():
            if industry_name in industry_results:
                original_score = holding_info.get('entry_momentum', 0)
                # 重新计算当前动量
                if industry_name in industry_results:
                    current_info = industry_results[industry_name]
                    current_return = current_info.get('avg_return', 0)
                    current_volatility = current_info.get('avg_volatility', 0.001)
                    current_score = current_return - self.volatility_penalty * current_volatility
                    
                    # 如果动量衰减超过30%，记一次衰减
                    if original_score > 0 and (original_score - current_score) / original_score > 0.3:
                        momentum_decay_count += 1
        
        # 如果有超过1/3的持仓行业动量显著衰减，触发再平衡
        if momentum_decay_count >= max(1, len(self.current_holdings) * 0.3):
            return True
            
        return True  # 默认按频率再平衡
    
    def execute_rotation(self, momentum_scores, current_weights, portfolio_capital, current_date, industry_results):
        """
        执行行业轮动
        返回: 新的权重分配方案
        """
        print(f"\n【行业动量轮动】执行日期: {current_date.strftime('%Y-%m-%d')}")
        print(f"  当前持仓行业数: {len(current_weights)}")
        
        # 1. 获取动量排名
        sorted_industries = sorted(
            [(ind, info['momentum_score']) for ind, info in momentum_scores.items()],
            key=lambda x: x[1],
            reverse=True
        )
        ...
        for industry_name, momentum_score in sorted_industries:
            if momentum_score > self.momentum_threshold:
                qualified_industries.append({
                    'name': industry_name,
                    'momentum_score': momentum_score,
                    'info': momentum_scores[industry_name]
                })
        
        print(f"  动量合格行业数: {len(qualified_industries)}")
        
        # 3. 确定目标行业（前N个）
        target_industry_count = min(self.max_industries, len(qualified_industries))
        target_industries = qualified_industries[:target_industry_count]
        
        if not target_industries:
            print("  警告: 无合格行业，保持当前权重")
            return current_weights
        
        # 4. 计算目标权重（基于动量分数）
        total_momentum = sum(ind['momentum_score'] for ind in target_industries)
        new_weights = {}
        
        for industry in target_industries:
            if total_momentum > 0:
                # 基于动量分数分配权重
                base_weight = industry['momentum_score'] / total_momentum
                
                # 应用约束
                base_weight = max(base_weight, 0.02)  # 最小2%
                base_weight = min(base_weight, 0.15)  # 最大15%
                
                new_weights[industry['name']] = base_weight
        ...
        # 6. 计算轮动变化
        rotation_changes = {}
        all_industries = set(list(current_weights.keys()) + list(new_weights.keys()))
        
        for industry in all_industries:
            old_weight = current_weights.get(industry, 0)
            new_weight = new_weights.get(industry, 0)
            
            if abs(new_weight - old_weight) > 0.001:  # 变化超过0.1%
                rotation_changes[industry] = {
                    'old': old_weight,
                    'new': new_weight,
        ...
        rotation_record = {
            'date': current_date,
            'target_industries': [ind['name'] for ind in target_industries],
            'new_weights': new_weights.copy(),
            'rotation_changes': rotation_changes,
            'momentum_scores': {ind['name']: ind['momentum_score'] for ind in target_industries}
        }
        self.rotation_log.append(rotation_record)
        
        # 8. 更新持仓状态
        for industry_name, weight in new_weights.items():
            if industry_name not in self.current_holdings:
                # 新进入的行业
                self.current_holdings[industry_name] = {
                    'entry_date': current_date,
                    'entry_momentum': momentum_scores[industry_name]['momentum_score'],
                    'current_weight': weight
                }
            else:
                # 已持有的行业，更新权重
                holding_info = self.current_holdings[industry_name]
                holding_days = (current_date - holding_info.get('entry_date', current_date)).days
                
                # 检查最小持有期
                if holding_days >= self.min_holding_days:
                    holding_info['current_weight'] = weight
                else:
                    # 未到最小持有期，保持原权重
                    new_weights[industry_name] = holding_info.get('current_weight', weight)
                    if industry_name in rotation_changes:
                        del rotation_changes[industry_name]
        
        # 9. 移除被调出的行业
        removed_industries = []
        for industry_name in list(self.current_holdings.keys()):
            if industry_name not in new_weights:
                removed_industries.append(industry_name)
                del self.current_holdings[industry_name]
        ...
        if rotation_changes:
            ...
            # 新增行业
            new_entries = [ind for ind, change in rotation_changes.items() 
                          if change['old'] == 0 and change['new'] > 0]
            if new_entries:
                print(f"    🆕 新增行业:")
                for ind in new_entries[:5]:  # 最多显示5个
                    info = momentum_scores.get(ind, {})
                    print(f"      - {ind}: 权重{new_weights[ind]:.1%} | "
                          f"动量{info.get('momentum_score', 0):.2f} | "
                          f"收益{info.get('recent_return', 0)*100:.1f}%")
                if len(new_entries) > 5:
                    print(f"      ... 等{len(new_entries)}个行业")
            
            # 剔除行业
            removed = [ind for ind, change in rotation_changes.items() 
                      if change['old'] > 0 and change['new'] == 0]
            if removed:
                print(f"    🗑️ 剔除行业:")
                for ind in removed[:5]:
                    print(f"      - {ind}: 原权重{rotation_changes[ind]['old']:.1%}")
                if len(removed) > 5:
                    print(f"      ... 等{len(removed)}个行业")
            
            # 权重调整
            adjusted = [ind for ind, change in rotation_changes.items() 
                       if change['old'] > 0 and change['new'] > 0]
            if adjusted:
                print(f"    📊 权重调整:")
                for ind in adjusted[:5]:
                    change_pct = rotation_changes[ind]['change'] * 100
                    if abs(change_pct) > 1:  # 只显示调整超过1%的
                        direction = "↑" if change_pct > 0 else "↓"
                        print(f"      - {ind}: {rotation_changes[ind]['old']:.1%} → "
                              f"{new_weights[ind]:.1%} ({direction}{abs(change_pct):.1f}%)")
    ...
    def save_rotation_report(self, filename=None):
        """保存轮动报告"""
        if filename is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"v7.4_行业轮动报告_{timestamp}.txt"
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            ...
            f.write("【轮动参数配置】\n")
            f.write(f"动量计算窗口: {self.momentum_lookback}天\n")
            f.write(f"再平衡频率: {self.rebalance_freq}天\n")
            f.write(f"动量入选门槛: {self.momentum_threshold}\n")
            f.write(f"最大轮动比例: {self.max_rotation_ratio*100:.0f}%\n")
            f.write(f"波动率惩罚系数: {self.volatility_penalty}\n")
            f.write(f"最小持有天数: {self.min_holding_days}\n")
            f.write(f"最大行业数量: {self.max_industries}\n\n")
            
            f.write("【轮动历史记录】\n")
            if not self.rotation_log:
                f.write("（无轮动记录）\n")
            else:
                for i, record in enumerate(self.rotation_log, 1):
                    f.write(f"\n第{i}次轮动 - 日期: {record['date']}\n")
                    f.write(f"  目标行业数: {len(record['target_industries'])}\n")
                    
                    # 显示行业详情
                    f.write("  行业详情:\n")
                    for j, industry in enumerate(record['target_industries'][:10], 1):  # 最多10个
                        weight = record['new_weights'].get(industry, 0) * 100
                        momentum = record['momentum_scores'].get(industry, 0)
                        f.write(f"    {j}. {industry}: 权重{weight:.1f}% | 动量{momentum:.2f}\n")
                    
                    if len(record['target_industries']) > 10:
                        f.write(f"    ... 等{len(record['target_industries'])}个行业\n")
            
            f.write("\n【当前持仓状态】\n")
            if not self.current_holdings:
                f.write("（无持仓）\n")
            else:
                for industry, info in self.current_holdings.items():
                    entry_date = info.get('entry_date', '未知')
                    if isinstance(entry_date, datetime):
                        holding_days = (datetime.now() - entry_date).days
                    else:
                        holding_days = 0
                    f.write(f"  - {industry}: 持仓{holding_days}天 | "
                          f"进入动量{info.get('entry_momentum', 0):.2f} | "
                          f"当前权重{info.get('current_weight', 0)*100:.1f}%\n")
        
        print(f"✓ 行业轮动报告已保存: {filename}")
        return filename
...
class IndustryCorrelationMonitor:
    ...
    def save_screening_report(self, filename=None):
        ...
        if filename is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"v7.4_行业筛选报告_{timestamp}.txt"
...
def run_portfolio_backtest_v7_4():
    """v7.4主函数：整合行业动量轮动"""
    cfg = CONFIG
    
    print("="*80)
    print("A股全行业配对交易策略 v7.4 - 整合行业动量轮动")
    print(f"初始资金: {cfg['PORTFOLIO_CAPITAL']:,.0f}")
    print(f"动量计算窗口: {cfg['MOMENTUM_ROTATION_PARAMS']['momentum_lookback']}天")
    print(f"再平衡频率: {cfg['MOMENTUM_ROTATION_PARAMS']['rebalance_freq']}天")
    print("="*80)
    ...
    # 【v7.4新增】初始化行业动量轮动器
    momentum_rotator = IndustryMomentumRotator(
        momentum_lookback=cfg['MOMENTUM_ROTATION_PARAMS']['momentum_lookback'],
        rebalance_freq=cfg['MOMENTUM_ROTATION_PARAMS']['rebalance_freq'],
        momentum_threshold=cfg['MOMENTUM_ROTATION_PARAMS']['momentum_threshold'],
        max_rotation_ratio=cfg['MOMENTUM_ROTATION_PARAMS']['max_rotation_ratio'],
        volatility_penalty=cfg['MOMENTUM_ROTATION_PARAMS']['volatility_penalty'],
        min_holding_days=cfg['MOMENTUM_ROTATION_PARAMS']['min_holding_days'],
        max_industries=cfg['MOMENTUM_ROTATION_PARAMS']['max_industries']
    )
    ...
    initial_weights = allocator.calculate_weights(industry_metrics)
    
    # 5. 【v7.4新增】行业动量轮动执行
    print(f"\n[5/8] 行业动量轮动执行...")
    
    # 模拟时间序列轮动
    # 在实际回测中，应该按日期循环，这里简化处理，展示轮动逻辑
    ...
    for i, rebalance_date in enumerate(rebalance_dates):
        print(f"\n  [{i+1}/{len(rebalance_dates)}] 轮动检查: {rebalance_date.strftime('%Y-%m-%d')}")
        
        # 检查是否应该执行轮动
        if momentum_rotator.should_rebalance(rebalance_date, industry_results):
            # 计算当前动量分数
            momentum_scores = momentum_rotator.calculate_momentum_scores(
                industry_returns_dict, rebalance_date
            )
            
            if momentum_scores:
                # 执行轮动
                new_weights = momentum_rotator.execute_rotation(
                    momentum_scores, current_weights, cfg['PORTFOLIO_CAPITAL'], rebalance_date, industry_results
                )
    ...
    for industry_name, weight in final_weights.items():
        if industry_name in industry_results:
    ...
    for name, w in sorted(final_weights.items(), key=lambda x: -x[1]):
        if name in industry_results:
            info = industry_results[name]
            cap = total_capital * w
            print(f"    {name}: 权重{w:.1%} | 资金{cap:,.0f} | "
                  f"收益{info['avg_return']*100:.1f}% | "
                  f"夏普{info['avg_sharpe']:.2f}")
    ...
    print(f"4. 行业动量轮动: 动态调整行业权重，捕捉行业轮动机会")
    print(f"5. 详细筛选报告: 保存完整筛选过程")
    print(f"6. 风险收益比权重: 科学分配行业权重")
    
    # 8. 保存结果
    print(f"\n[8/8] 保存回测结果...")
    
    # 保存轮动报告
    rotation_report_file = momentum_rotator.save_rotation_report()
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_file = f"v7.4_组合回测结果_{timestamp}.csv"
    results_df.to_csv(results_file, index=False, encoding='utf-8-sig')
    print(f"✓ 回测结果已保存: {results_file}")
    
    config_file = f"v7.4_回测配置_{timestamp}.json"
    ...
    print(f"✓ 行业轮动报告: {rotation_report_file}")
