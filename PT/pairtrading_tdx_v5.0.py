# === 完整的优化版代码（实现动态对冲比率 + 动态协整监控 + 自适应交易带 + 无交易区间）===
# === 第一部分：导入必要的库 ===
import struct
import pandas as pd
import numpy as np
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("A股配对交易策略（优化版v2：动态对冲比率 + 动态协整监控 + 自适应交易带 + 无交易区间）")
    ...
    if all_results:
        ...
        for i, r in enumerate(sorted_results):
            ...
            print(f"{i+1}. {r['pair']}: 收益率={r['return']:.2%}, 对冲比率={r['hedge_ratio']:.4f}, "
                  f"交易次数={r['trades']}, 胜率={r['win_rate']:.1f}%{cooling_str}, 状态={status}")
        ...
        plt.title(f'{TARGET_INDUSTRY}行业配对交易策略样本外回测（优化版v2，前{TOP_N_PAIRS}对）\n'
                  f'动态对冲比率 + 动态协整监控 + 自适应交易带 + 无交易区间 | 总收益率: {overall_return:.2%}', 
