# === 完整的优化版代码（实现动态对冲比率 + 动态协整监控 + 自适应交易带 + 无交易区间 + 失效后净值优化）===
# === 第一部分：导入必要的库 ===
import struct
import pandas as pd
import numpy as np
...
def get_industry_stocks_from_local(industry_name, local_file_path=None):
    ...
    try:
        ...
        return []
# === 第三部分：A股适配的强弱轮动策略（优化版v2）===
class ASharePairsTradingStrategyOptimizedV2:
    """A股配对交易策略（强弱轮动，不允许融券）- 优化版v2
    新增功能：
    1. 自适应交易带：入场/出场阈值与价差滚动标准差挂钩
    2. 无交易区间：平仓后设置冷却期，防止频繁交易
    3. 失效后净值优化：配对失效后，资金按无风险利率增长或保持现金
    """
    def __init__(self, initial_capital=1000000, commission_rate=0.0003, 
                 ...
                 risk_free_rate=0.02              # 新增：无风险年化收益率，用于失效后净值计算
                 ):
                ...
                if z > adaptive_entry:  # stock1相对高估，stock2低估
                    ...
                    if volume > 0:
                        success, _, _ = self.buy_stock(date, stock2_code, price2, volume, 
                                                      reason=f"开仓-买入低估股({stock2_code}), z={z:.2f}, 入场阈值={adaptive_entry:.2f}")
                        if success:
                    ...
                    if volume > 0:
                        success, _, _ = self.buy_stock(date, stock1_code, price1, volume, 
                                                      reason=f"开仓-买入低估股({stock1_code}), z={z:.2f}, 入场阈值={adaptive_entry:.2f}")
                        if success:
...
if __name__ == "__main__":
    ...
    # 自适应交易带参数
    ENTRY_MULTIPLIER = 1.5    # 入场阈值乘数 (默认1.8，相当于1.8σ)
    EXIT_MULTIPLIER = 0.3     # 出场阈值乘数 (默认0.4，相当于0.4σ)
    STOP_LOSS_MULTIPLIER = 2.0  # 止损阈值乘数 (默认2.2，相当于2.2σ)
    REBALANCE_MULTIPLIER = 0.8  # 轮动阈值乘数 (默认0.7，相当于0.7σ)
    ...
    print("="*60)
    print("A股配对交易策略（优化版v2：动态对冲比率 + 动态协整监控 + 自适应交易带 + 无交易区间 + 失效后净值优化）")
    ...
    sample_in_df.to_csv(f'A股配对_样本内筛选结果_优化版v2_前{TOP_N_PAIRS}对.csv', index=False, encoding='utf-8-sig')
    print(f"样本内筛选结果已保存到: A股配对_样本内筛选结果_优化版v2_前{TOP_N_PAIRS}对.csv")
    ...
    for idx, pair in enumerate(top_pairs):
        ...
        strategy = ASharePairsTradingStrategyOptimizedV2(
        ...
        if result is not None:
            # 计算最终收益率
            # 注意：最终价值已经从equity_curve的最后一个值获得，包含了失效后的净值计算
            ...
            trades_df.to_csv(f'A股配对_交易记录_{stock1}_{stock2}_优化版v2_前{TOP_N_PAIRS}对.csv', index=False, encoding='utf-8-sig')
            
            # 保存每日净值
            if strategy.equity_curve:
                equity_df = pd.DataFrame(strategy.equity_curve, columns=['date', 'value'])
                equity_df.set_index('date', inplace=True)
                equity_df.to_csv(f'A股配对_每日净值_{stock1}_{stock2}_优化版v2_前{TOP_N_PAIRS}对.csv', encoding='utf-8-sig')
            
            # 保存每日记录（包含更多细节）
            daily_df = pd.DataFrame(strategy.daily_records)
            daily_df.set_index('date', inplace=True)
            daily_df.to_csv(f'A股配对_每日记录_{stock1}_{stock2}_优化版v2_前{TOP_N_PAIRS}对.csv', encoding='utf-8-sig')
    
    # 输出汇总结果
    if all_results:
        print(f"\n" + "="*60)
        print(f"样本外回测汇总结果（优化版v2，前{TOP_N_PAIRS}对）")
        print("="*60)
        ...
        for i, r in enumerate(sorted_results):
            ...
            print(f"{i+1}. {r['pair']}: 收益率={r['return']:.2%}, 对冲比率={r['hedge_ratio']:.4f}, "
                  f"交易次数={r['trades']}, 胜率={r['win_rate']:.1f}%{cooling_str}{invalid_info}, 状态={status}")
        ...
        plt.title(f'{TARGET_INDUSTRY}行业配对交易策略样本外回测（优化版v2，前{TOP_N_PAIRS}对）\n'
                  f'动态对冲比率 + 动态协整监控 + 自适应交易带 + 无交易区间 + 失效后净值优化 | 总收益率: {overall_return:.2%}', 
        ...
        plt.tight_layout()
        plt.savefig(f'{TARGET_INDUSTRY}配对_样本外回测净值曲线_优化版v2_前{TOP_N_PAIRS}对.png', dpi=300, bbox_inches='tight')
        plt.show()
        ...
        summary_df.to_csv(f'{TARGET_INDUSTRY}配对_样本外回测汇总_优化版v2_前{TOP_N_PAIRS}对.csv', index=False, encoding='utf-8-sig')
        print(f"\n详细结果已保存到: {TARGET_INDUSTRY}配对_样本外回测汇总_优化版v2_前{TOP_N_PAIRS}对.csv")
    else:
        print("\n没有成功的回测结果")