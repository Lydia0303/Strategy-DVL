# === 完整的优化版代码（实现动态对冲比率 + 动态协整监控 + 增强自适应交易带 + 强化无交易区间 + 失效后净值优化 + 浮动止损）===
# === 第一部分：导入必要的库 ===
import struct
import pandas as pd
import numpy as np
...
class ASharePairsTradingStrategyOptimizedV3:
    ...
    def calculate_adaptive_thresholds(self, spread_std, entry_threshold=1.5, exit_threshold=0.3, 
        ...
        if not self.use_adaptive_thresholds or spread_std <= 0:
            # 如果不使用自适应阈值，或价差标准差为0（无波动性），返回固定阈值（备用方案）
        ...
        # 限制调整范围，避免阈值过宽或过窄
        adjustment = max(0.5, min(2.0, volatility_ratio))
        
        # 根据波动率调整阈值
        adaptive_entry = self.entry_threshold_multiplier * adjustment
        adaptive_exit = self.exit_threshold_multiplier * adjustment
        adaptive_stop_loss = self.stop_loss_multiplier * adjustment * 1.1  # 在波动调整基础上再放宽10%
...
if __name__ == "__main__":
    ...
    for idx, pair in enumerate(top_pairs):
        ...
        if result is not None:
            ...
            daily_df.to_csv(f'A股配对_每日记录_{stock1}_{stock2}_优化版v3_前{TOP_N_PAIRS}对.csv', encoding='utf-8-sig')
    ...
    if all_results:
        ...
        for i, r in enumerate(sorted_results):
            ...
            print(f"{i+1}. {r['pair']}: 收益率={r['return']:.2%}, 对冲比率={r['hedge_ratio']:.4f}, "
                  ...
                  f"{cooling_str}{float_stop_str}{invalid_info}, 状态={status}")
        ...
        plt.title(f'{TARGET_INDUSTRY}行业配对交易策略样本外回测（优化版v3，前{TOP_N_PAIRS}对）\n'
                  f'动态对冲比率 + 动态协整监控 + 增强自适应交易带 + 强化无交易区间 + 失效后净值优化 + 浮动止损 | 总收益率: {overall_return:.2%}', 
        ...
        print("   - 提高 entry_threshold_multiplier (如从1.8提高到2.0)")
        print("   - 降低 rebalance_threshold_multiplier (如从0.8降低到0.6)")
        ...
        print("\n3. 如果浮动止损触发过于频繁，可尝试:")
        print("   - 提高 float_stop_threshold (如从0.5提高到0.8)")
