# === 完整的优化版代码（实现动态对冲比率 + 动态协整监控 + 自适应交易带 + 无交易区间 + VaR头寸管理）- 简化版（无因子中性化）===
# === 第一部分：导入必要的库 ===
import struct
import pandas as pd
import numpy as np
...
# === 第三部分：A股适配的强弱轮动策略（优化版v3简化版，无因子中性化）===
class ASharePairsTradingStrategySimplifiedV3:
    """A股配对交易策略（强弱轮动，不允许融券）- 优化版v3简化版
    包含功能：
    1. 动态对冲比率：基于OLS回归计算对冲比率
    2. 动态协整检验：定期检验协整关系，关系破裂时强制平仓
    3. 自适应交易带：入场/出场阈值与价差滚动标准差挂钩
    4. 无交易区间：平仓后设置冷却期，防止频繁交易
    5. 基于VaR的头寸管理：根据价差风险动态调整仓位
    （简化版：移除了因子中性化检查）
    """
    ...
    def execute_strategy(self, stock1_code, stock2_code, stock1_data, stock2_data, 
        ...
        """
        执行强弱轮动策略 (优化版v3简化版，包含VaR头寸管理但不含因子中性化)
...
# === 第四步：主程序（优化版v3简化版，包含VaR头寸管理但不含因子中性化）===
if __name__ == "__main__":
    ...
    print("="*60)
    print("A股配对交易策略（优化版v3简化版：动态对冲比率 + 动态协整监控 + 自适应交易带 + 无交易区间 + VaR头寸管理）")
    ...
    for stock1, stock2, ssd_dist, correlation in candidate_pairs[:20]:
        ...
        hedge_ratio, intercept, coint_p, adf_p, spread_mean, spread_std = result
    ...
    sample_in_df.to_csv(f'A股配对_样本内筛选结果_优化版v3简化版_前{TOP_N_PAIRS}对.csv', index=False, encoding='utf-8-sig')
    print(f"样本内筛选结果已保存到: A股配对_样本内筛选结果_优化版v3简化版_前{TOP_N_PAIRS}对.csv")
    ...
    for idx, pair in enumerate(top_pairs):
        ...
        strategy = ASharePairsTradingStrategySimplifiedV3(
        ...
        if result is not None:
            ...
            avg_var = 0
            avg_cvar = 0
            if strategy.daily_records:
                var_values = [rec.get('var_value', 0) for rec in strategy.daily_records if rec.get('var_value', 0) > 0]
                cvar_values = [rec.get('cvar_value', 0) for rec in strategy.daily_records if rec.get('cvar_value', 0) > 0]
            ...
            print(f"    平均VaR: {avg_var:.4%}, 平均CVaR: {avg_cvar:.4%}")
            ...
            trades_df.to_csv(f'A股配对_交易记录_{stock1}_{stock2}_优化版v3简化版_前{TOP_N_PAIRS}对.csv', index=False, encoding='utf-8-sig')
            
            # 保存每日净值
            if strategy.equity_curve:
                equity_df = pd.DataFrame(strategy.equity_curve, columns=['date', 'value'])
                equity_df.set_index('date', inplace=True)
                equity_df.to_csv(f'A股配对_每日净值_{stock1}_{stock2}_优化版v3简化版_前{TOP_N_PAIRS}对.csv', encoding='utf-8-sig')
    
    # 输出汇总结果
    if all_results:
        print(f"\n" + "="*60)
        print(f"样本外回测汇总结果（优化版v3简化版，前{TOP_N_PAIRS}对）")
        print("="*60)
        ...
        for i, r in enumerate(sorted_results):
            ...
            var_str = f", 平均VaR: {r['avg_var']:.4%}" if r.get('avg_var') and r['avg_var'] > 0 else ""
            print(f"{i+1}. {r['pair']}: 收益率={r['return']:.2%}, 对冲比率={r['hedge_ratio']:.4f}, "
                  f"交易次数={r['trades']}, 胜率={r['win_rate']:.1f}%{var_str}{cooling_str}, 状态={status}")
        ...
        plt.title(f'{TARGET_INDUSTRY}行业配对交易策略样本外回测（优化版v3简化版，前{TOP_N_PAIRS}对）\n'
                  f'动态对冲比率 + 动态协整监控 + 自适应交易带 + 无交易区间 + VaR头寸管理 | 总收益率: {overall_return:.2%}', 
        ...
        plt.tight_layout()
        plt.savefig(f'{TARGET_INDUSTRY}配对_样本外回测净值曲线_优化版v3简化版_前{TOP_N_PAIRS}对.png', dpi=300, bbox_inches='tight')
        plt.show()
        ...
        summary_df = pd.DataFrame([{
            ...
            '胜率': r['win_rate'],
            '夏普比率': r['sharpe'],
            '平均VaR': r.get('avg_var', 0),
            '平均CVaR': r.get('avg_cvar', 0),
            '协整检验次数': r['coint_checks'],
            '冷却期触发次数': r.get('cooling_triggers', 0),
            '配对最终状态': '有效' if r['pair_valid'] else '失效'
        } for r in all_results])
        
        summary_df.to_csv(f'{TARGET_INDUSTRY}配对_样本外回测汇总_优化版v3简化版_前{TOP_N_PAIRS}对.csv', index=False, encoding='utf-8-sig')
        print(f"\n详细结果已保存到: {TARGET_INDUSTRY}配对_样本外回测汇总_优化版v3简化版_前{TOP_N_PAIRS}对.csv")
        ...
        if len(all_results) > 1:
            ...
            # 计算VaR与收益的相关系数
            var_values = [r.get('avg_var', 0) for r in all_results]
            returns = [r['return'] for r in all_results]
            
            if len(var_values) > 1 and len(returns) > 1:
                corr_var_return = np.corrcoef(var_values, returns)[0, 1]
                print(f"平均VaR与收益的相关系数: {corr_var_return:.4f}")
                
                if corr_var_return < -0.3:
                    print(f"  ✓ VaR与收益呈较强负相关，说明VaR头寸管理有效（高风险低收益，应降低头寸）")
                elif corr_var_return > 0.3:
                    print(f"  ⚠️ VaR与收益呈较强正相关，需重新评估VaR头寸管理参数")
                else:
                    print(f"  → VaR与收益相关性较弱")
            
            # 按VaR分组
            low_var_pairs = [r for r in all_results if r.get('avg_var', 0) < 0.01]
            medium_var_pairs = [r for r in all_results if 0.01 <= r.get('avg_var', 0) < 0.02]
            high_var_pairs = [r for r in all_results if r.get('avg_var', 0) >=0.02]
            
            print(f"\n按平均VaR分组统计:")
            print(f"  低VaR组(<1%): {len(low_var_pairs)}对, 平均收益: {np.mean([r['return'] for r in low_var_pairs]):.2%}" if low_var_pairs else "  低VaR组: 无配对")
            print(f"  中VaR组(1%-2%): {len(medium_var_pairs)}对, 平均收益: {np.mean([r['return'] for r in medium_var_pairs]):.2%}" if medium_var_pairs else "  中VaR组: 无配对")
            print(f"  高VaR组(≥2%): {len(high_var_pairs)}对, 平均收益: {np.mean([r['return'] for r in high_var_pairs]):.2%}" if high_var_pairs else "  高VaR组: 无配对")
            ...
            print(f"  最大夏普比率: {max(sharpe_ratios):.4f}" if sharpe_ratios else "  无数据")
            print(f"  最小夏普比率: {min(sharpe_ratios):.4f}" if sharpe_ratios else "  无数据")
            
            # VaR头寸管理效果评估
            if USE_VAR_SIZING:
                print(f"\nVaR头寸管理效果:")
                # 比较VaR调整前后的头寸比例
                var_pairs = [r for r in all_results if r.get('avg_var', 0) > 0]
                if var_pairs:
                    avg_var = np.mean([r.get('avg_var', 0) for r in var_pairs])
                    # 假设最大风险预算为MAX_RISK_BUDGET
                    # VaR头寸比例 = min(MAX_RISK_BUDGET / VaR, max_position_ratio)
                    theoretical_position_ratio = min(MAX_RISK_BUDGET / (avg_var + 1e-6), 0.5)
                    print(f"  平均VaR: {avg_var:.4%}")
                    print(f"  基于VaR的理论头寸比例: {theoretical_position_ratio:.2%}")
                    print(f"  固定头寸比例: {0.5:.2%} (对比)")
                    if theoretical_position_ratio < 0.5:
                        print(f"  ✓ VaR头寸管理降低了头寸暴露，从{0.5:.2%}降至{theoretical_position_ratio:.2%}")
                    else:
                        print(f"  → VaR头寸管理未降低头寸暴露")
    else:
        print("\n没有成功的回测结果")