...
if __name__ == "__main__":
    ...
    OUT_SAMPLE_END = "2025-07-23"    # 样本外结束
    ...
    sample_in_df.to_csv('A股配对_样本内筛选结果_优化版_前2对.csv', index=False, encoding='utf-8-sig')
    print("样本内筛选结果已保存到: A股配对_样本内筛选结果_优化版_前2对.csv")
    ...
    for idx, pair in enumerate(top_pairs):
        ...
        if result is not None:
            ...
            trades_df.to_csv(f'A股配对_交易记录_{stock1}_{stock2}_优化版_前2对.csv', index=False, encoding='utf-8-sig')
            
            # 保存每日净值
            if strategy.equity_curve:
                equity_df = pd.DataFrame(strategy.equity_curve, columns=['date', 'value'])
                equity_df.set_index('date', inplace=True)
                equity_df.to_csv(f'A股配对_每日净值_{stock1}_{stock2}_优化版_前2对.csv', encoding='utf-8-sig')
