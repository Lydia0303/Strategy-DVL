# === 完整的优化版代码（AKShare动态获取股票列表 + 只回测前2对） ===
# === 第一部分：导入必要的库 ===
import struct
import pandas as pd
import numpy as np
...
def get_industry_stocks_from_akshare(industry_name="银行"):
    """
    ...
    try:
        ...
        # 提取股票代码列
        # 注意：AKShare 接口返回的股票代码可能包含后缀，如 '601288.SH'，我们只需要前面的数字代码
...
# === 第四步：主程序（从AKShare获取行业股票 + 样本内选股 + 只回测前2对） ===
if __name__ == "__main__":
    ...
    industry_stocks = get_industry_stocks_from_akshare(TARGET_INDUSTRY)
    ...
    if all_results:
        ...
        plt.title(f'{TARGET_INDUSTRY}行业配对交易强弱轮动策略样本外回测（前{TOP_N_PAIRS}对）\n'
        ...
        plt.tight_layout()
        plt.savefig(f'{TARGET_INDUSTRY}配对_样本外回测净值曲线_前{TOP_N_PAIRS}对.png', dpi=300, bbox_inches='tight')
        plt.show()
        ...
        summary_df.to_csv(f'{TARGET_INDUSTRY}配对_样本外回测汇总_前{TOP_N_PAIRS}对.csv', index=False, encoding='utf-8-sig')
        print(f"\n详细结果已保存到: {TARGET_INDUSTRY}配对_样本外回测汇总_前{TOP_N_PAIRS}对.csv")
    else:
        print("\n没有成功的回测结果")